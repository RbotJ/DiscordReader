"""
Setup Service Layer (Adapter)

Provides business logic for processing and storing trade setups.
Acts as a mediator between API controllers and the repository adapter.
"""
import logging
import hmac
import hashlib
...
class SetupServiceAdapter:
    """Service for processing trade setup messages using the adapter pattern."""
    ...
    def process_webhook(payload: Dict[str, Any], signature: Optional[str] = None) -> Tuple[bool, Dict[str, Any]]:
        ...
        if signature:
            is_valid = SetupServiceAdapter._verify_signature(payload, signature)
            if not is_valid:
        ...
        try:
            ...
            # Save to database
            message_id = SetupRepositoryAdapter.save_setup_message(setup_message)
    ...
    def get_recent_setups(limit: int = 10) -> List[TradeSetupMessage]:
        ...
        """
        return SetupRepositoryAdapter.get_latest_setups(limit)
    ...
    def get_setups_for_symbol(symbol: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        return SetupRepositoryAdapter.get_setups_by_symbol(symbol, limit)
