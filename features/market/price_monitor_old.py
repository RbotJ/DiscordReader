...
def publish_price_update(self, symbol: str, price: float):
    """Publish a price update to database."""
    try:
        from common.db_models import PriceUpdateModel
        ...
        # Publish event
        publish_event(EventChannels.PRICE_UPDATE, {
            'symbol': symbol,
            'price': price,
            'timestamp': datetime.utcnow().isoformat()
        })
    except Exception as e:
        logger.error(f"Error publishing price update to database: {e}")